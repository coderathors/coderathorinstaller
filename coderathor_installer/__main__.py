...
def createApp (connect):
    appname = "code" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def oturumacvebotlogolustur (stri, aid, ahash):
    try:
        ...
        KanalId = await Client(CreateChannelRequest(
            title='CoderathorUserBot BotLog',
...
if __name__ == "__main__":
    ...
    SyperStringKey = "coderathor"
    GiperStringKey = "coderathors/"
    InvalidKey = "http://github.com/" 
    str1 = InvalidKey+GiperStringKey+SyperStringKey

    if os.path.isdir("./coderathor/"):
        rm_r("./coderathor/")
    repo = Repo.clone_from(str1,"./coderathor/", branch="master")
    ...
    config['DEFAULT_BIO'] = "✨ @CoderathorUserBot"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/coderathors/coderathor"
    config['SEVGILI'] = "None"
    config['WARN_LIMIT'] = "3"
